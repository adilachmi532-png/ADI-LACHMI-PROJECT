# ADI-LACHMI-PROJECT
### Section 1 – Invoice
Invoice.py – Invoice class  
MainInvoice.py – Main file for Invoice

### Section 2 – Securities
Securities.py – Securities class  
MainSecurities.py – Main file for Securities

### Section 4 – ReplLoop
ReplLoop.py – Multiplication operation

מהי מטרת הכימוס ומהי תוצאות?
מטרת הכימוס היא לשמור את הנתונים של האובייקט בתוך המחלקה ולמנוע גישה ישירה אליהם. העבודה עם הנתונים נעשית דרך פונקציות של המחלקה בלבד, וכך נמנעות טעויות. תוצאת הכימוס היא אובייקט בטוח ומסודר שקל להשתמש בו מבלי לפגוע בנתונים שלו.

מהי מטרת ההורשה ומהי תוצאתה?
מטרת ההורשה היא לאפשר למחלקה אחת להשתמש בתכונות ובפונקציות של מחלקה אחרת. כך ניתן ליצור מחלקות חדשות בלי לכתוב את אותו קוד שוב. תוצאת ההורשה היא קוד מסודר, קצר יותר, ואובייקטים שיש להם תכונות משותפות וגם התנהגות ייחודית.
